 ...
 *
 * @brief A custom graph class which extends a boost::adjacency_list with support for 'clustering', dcm properties
 * and support for filtering
 * 
 ...
 * part of the graph in a non-disruptive way. The need for grouping arises from the fact, that fully constraint
 ...
 *
 * The ClusterGraph extends the boost graph in three ways:
 * First it allows to specify properties which are stored at vertices, edges and the
 * graph itsef in a dcm conformant manner. There are functions to compfortable access and set them which mimic the 
 * standart property interface
 * Second, and most important, it allows subclustering. That means creating independend graphs as
 * child, where every vertex can be accessed by edges from the parent graph. 
 * The last extension is the pssibility to filter a graph for different groups and hence to allow boost algorithms
 * to work only on a subset of the graph. 
 * A ClusterGraph is noncopyable, it is therefore always passed around as shared_ptr.
 * 
 ...
 * You can see how a global descriptor was added to every entity, but you see something else too: Some strange global
 ...
 * own type and holds the global edge descriptor, so don't confuse it with a descriptor itself. The whole purpose of
 ...
 * that the boost local descriptors will stay unchanged as long as you do not move the entity out of the graph,
 * thats a result of the listS usage and therefore the local descriptors beeing void pointers. That would be diffrent
 ...
 * Thats the single reason we introduced the stacked global edges in the first place, to allow a arbitrary number of
 ...
 * 
 * \subsection properties Adding properties
 * 
 * It is possible to at data to the ClusterGraph's entitys by attatching multiple \ref Property 's to them. This includes 
 * local and global edges, local and global vertices and clusters. The vertex and edge properties are intended to be used 
 * in boost graph algorithms and shall be used in combination with the \ref property_map interface, but also to store data
 * of any kind in the graph. It's also possible to add properties to a whole clustergraph. This is needed to give the 
 * subclusters a meaning of some kind. With attached properties it's possible to differentiate use cases and distuingish
 * the relevant cluster.
 * 
 * To add properties to the possible places they need to be passed as mpl::vectors in the appropriate template
