...
template<typename Sys>
MES<Sys>::MES(Cluster& cl, int par, int eqn) : Base(par, eqn), m_cluster(cl) {
...
template<typename Sys>
SystemSolver<Sys>::Rescaler::Rescaler(Cluster& c, Mes& m) : cluster(c), mes(m), rescales(0) {
...
template<typename Sys>
void SystemSolver<Sys>::solveCluster(Cluster& cluster, Sys& sys) {
